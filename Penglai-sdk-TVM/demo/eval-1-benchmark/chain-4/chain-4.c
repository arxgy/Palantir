...
{  
  char *elf_file_name = "/root/chain-3";
  ...
  while (retval == 0)
  {
    ...
    {
      eapp_print("[pe] [chain-4] eapp_run_enclave returned! \n");
      break;
    }
    /* we reuse the [return reason] as [resume reason] */
    if (retval)
    {
      eapp_print("[pe] [chain-4] eapp_inspect_enclave return_value non-zero: [%d]\n", return_value);
      break;
    }
  ...
  /* exit successfully */
  eapp_print("[pe] [chain-4] hello world!\n");
  EAPP_RETURN(0);
}
